...
import ItemSelectorCheckbox from "../ItemCheckbox/ItemCheckbox"
...
const SelectItemsDialogForObserver = () => {
    ...
    const checkElementState = (id: number) => {
        const item = store.selectedItems.find(item => item.itemId === id)
    ...
    const disableCheckIfConditionsAreMet = (id: number) => {
        ...
        return item ? false : store.selectedItems.length === 3 ? true : false
    }

    const renderElements = () => {

        const items = Array.from({ length: 300 - store.searchObject.filterValue }, (_, index) => {
            return {
                itemId: store.searchObject.filterValue + index + 1,
                itemText: `Element ${store.searchObject.filterValue + index + 1}`,
                checked: checkElementState(store.searchObject.filterValue + index + 1),
                disabled: disableCheckIfConditionsAreMet(store.searchObject.filterValue + index + 1)
            }
        });

        for (let i = items.length - 1; i >= 0; i--) {
            if (!items[i].itemText.includes(store.searchObject.searchValue)) {
                items.splice(i, 1)
            }
        }
    ...
    const filterValue = (event: React.ChangeEvent<HTMLSelectElement>) => {
        store.setItemsFilter(parseInt(event.currentTarget.value))
    ...
    const changeSearchValue = (event: React.ChangeEvent<HTMLInputElement>) => {
        store.setSearcValue(event.currentTarget.value)
                ...
                <div className="dialog df fdc g15px">
                    ...
                    <form className="df jcsb g15px fdcResMob">
                        <div className="df g7px pRel oh f1 jcsb">
                            ...
                            <input type="text" id="searchItemsInput" className="dialog_input w100mob" value={store.searchObject.searchValue} onChange={changeSearchValue} />
                        ...
                        <div className="df g7px pRel oh f1 jcsb">
                            ...
                            <select id="searchItemsFilter" className="dialog_input w100mob" onChange={filterValue}>
                    ...
                    <div className="df g15px fdc mt6px">
                        ...
                        <div className="df g10px fww">
                            {store.selectedItems.length > 0 ? (
                                store.selectedItems.map((item: Item, index: number) => (
